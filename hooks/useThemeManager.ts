...
const useThemeManager = (theme?: Theme) => {
    useEffect(() => {
        if (theme) {
            ...
            // Fix: Set content colors using the correct HSL format for DaisyUI.
            // This ensures text on colored backgrounds is readable.
            root.style.setProperty('--pc', '0 0% 100%'); // primary-content (white)
            root.style.setProperty('--sc', '0 0% 100%'); // secondary-content (white)
            root.style.setProperty('--ac', '0 0% 100%'); // accent-content (white)
        }
    }, [theme]);
};
